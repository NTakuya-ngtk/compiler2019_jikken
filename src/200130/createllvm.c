...
void init_procCallstack(){
	callstack.top = 0;
	return;
}

LLVMcode* procCallpop(){
	LLVMcode* tmp;
	tmp = callstack.element[callstack.top];
	callstack.top --;
	return tmp;
}

void procCallpush(LLVMcode* x){
	callstack.top ++;
	callstack.element[callstack.top] = x;
	return;
}
